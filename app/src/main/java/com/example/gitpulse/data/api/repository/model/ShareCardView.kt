...
class ShareCardView(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun drawFrostedCard(canvas: Canvas, w: Float, h: Float) {
        ...
        cardPaint.color = if (isLight)
            Color.argb(180, 255, 255, 255)
        else
            Color.argb(160, 20, 27, 34)
        ...
        val borderPaint = Paint().apply {
            ...
            strokeWidth = 1.5f
            isAntiAlias = true
            color = if (isLight) Color.argb(80, 255, 255, 255)
    ...
    private fun drawContent(canvas: Canvas, w: Float, h: Float) {
        ...
        canvas.drawText(
            "GitPulse",
            w * 0.12f, h * 0.14f,
            textPaint(accentColor, w * 0.034f, mono = true)
        )
        ...
        if (avatarBitmap != null) {
            ...
            canvas.drawText(
                ...
                textPaint("#FFFFFF", avatarR * 1.1f, bold = true)
                    .also { it.textAlign = Paint.Align.CENTER }
            )
        }


        // username — auto shrink if too long
        ...
        canvas.drawText(
            "Developer",
            w * 0.38f, h * 0.25f + w * 0.075f,
            textPaint(textMuted, w * 0.034f, mono = true)
        )
        ...
        canvas.drawText(
            longestStreak,
            w * 0.12f, statY,
            textPaint(accentColor, statNumSize, bold = true, mono = true)
        )
        canvas.drawText(
            "streak 🔥",
            w * 0.12f, statY + statNumSize * 0.7f,
            textPaint(textMuted, statLabelSize, mono = true)
        )
        ...
        canvas.drawText(
            "top language",
            w * 0.45f, statY + statNumSize * 0.7f,
            textPaint(textMuted, statLabelSize, mono = true)
        )
        ...
        // dev score — hero number
        canvas.drawText(
            devScore,
            w * 0.12f, h * 0.80f,
            textPaint(mainText, w * 0.18f, bold = true, mono = true)
        )
        canvas.drawText(
            "dev score",
            w * 0.12f, h * 0.80f + w * 0.055f,
            textPaint(accentColor, w * 0.036f, mono = true)
        )
    ...
    private fun drawGradientBg(canvas: Canvas, w: Float, h: Float) {
        val gradient = LinearGradient(
            ...
            intArrayOf(
                ...
                Color.parseColor("#B8C0FF"),
                Color.parseColor("#FFD6E0")
            ),
    ...
    private fun drawSakuraBg(canvas: Canvas, w: Float, h: Float) {
        val gradient = LinearGradient(
            0f, 0f, 0f, h,
            intArrayOf(
                Color.parseColor("#FFE4E8"),
                Color.parseColor("#FFCCD5"),
                Color.parseColor("#FDB8C4")
            ),
            floatArrayOf(0f, 0.5f, 1f),
            Shader.TileMode.CLAMP
        )
